...
**Model sources:** _Apollo Saturn V Launch Vehicle_ by **devPilot** on Sketchfab, **CC BY** (attribution required). ~8.8 MB, ~106k tris. Plus _N1 rocket_ by **Scilence**, **CC BY 4.0** (~6 MB) for the side-by-side comparison, and _Statue of Liberty_ by **Maurice Svay**, **CC BY 4.0** (decimated to ~4 MB) for the Compare button — every credit renders bottom-left while its model is up. (Replaced the original NASA `.glb`, which was one fused mesh — see git history.)
