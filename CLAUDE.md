...
export type Hotspot = {
  ...
  homecoming?: string; // label of a button at the card's foot that plays HOMECOMING ("What came home?")
};
...
src/
  ...
  scene/
    ...
    csm.ts                     # Apollo 11's CSM "Columbia" from primitives, in metres (AOH station numbers + flight photos): SPS bell, SM (ECS radiators, RCS quads, scimitars, high-gain antenna, "UNITED STATES" + flag), EPS fairing, umbilical, the foil-wrapped CM (hatch, windows, RCS ports, docking ring + probe). Merged into one mesh per material per module — groups `CSM_SM` (with the booms) and `CSM_CM`, so the SM can go on its own — ~25 draw calls; the high-gain antenna and EVA floodlight merge onto their own pivots (`CSM_HGA_Pivot` / `CSM_Floodlight_Pivot`, stowed at `HGA_STOWED` / `FLOOD_STOWED`) so they can ride folded and swing out. `buildLMTunnel` is the LM's docking tunnel — it travels with the LM. Built once and cached
    ...
    Homecoming.tsx             # the spacecraft card's "What came home?": fades the vehicle away part by part in Apollo 11's order (`HOMECOMING` in hotspots.ts), then closes in on the command module and turns it. A pure function of one timeline `u`, like Spacecraft.tsx. Portals the detailed CSM + LM into `Spacecraft_Top` through `useSpacecraftParts` / `DockedLM` / `rigFor` (exported from Spacecraft.tsx) — the LM sits lower than in TD&E, its tunnel just under the SPS bell, because here the bell never moves out of its way
    fade.ts                    # fades whole parts out and back: `fader(nodes).set(alpha)` / `.restore()`, per-mesh transparent clones under their own userData key (isolate.ts's bookkeeping, kept apart). Clones persist between runs so their shaders stay compiled
...
- [x] **Spacecraft button (transposition, docking and extraction)** — fourth HUD button (and `S`). Plays Apollo 11's TD&E out of the Saturn V's own nose, straight through in ~25 s (`Spacecraft.tsx`). The escape tower and boost cover jettison (really 3 minutes after launch). The CSM separates while the adapter's four panels swing open and fly off. The CSM backs out 12 m and turns end over end, with the high-gain antenna and floodlight swinging out. It docks with the LM still sitting on the S-IVB and pulls it out. A last beat skips ahead to lunar orbit, where the LM's legs come down (Apollo 11: "Landing Gear Deploy, Fire", 098:14:35 GET, two hours before undocking) and the pair rolls slowly. **The roll is presentation licence:** the barbecue roll was only flown with the gear stowed. **The LM never flew itself out** — the CSM did all the flying.
      ...
      **Pre-talk:** after loading, press S then Esc once. The first press compiles the spacecraft's shaders — better before the audience than during. (Then warm up What came home? the same way — see its entry below.)
...
- [x] **"What came home?" — the spacecraft card's button** — a hotspot can carry `homecoming: "<label>"`, and its card gets a button at its foot: the HUD's glass pill (`hud-btn`), a size up. On 04 it plays `HOMECOMING` (`hotspots.ts`, rendered by `Homecoming.tsx`). The camera pulls back to the wide shot, and the card, brackets and progress strip go. Then the vehicle lets go of every part that didn't come back, one beat each, in Apollo 11's order: the S-IC + interstage ring (2:42 / 3:12), the escape tower + boost cover (3:17), the S-II + the interstage above it (9:09), the S-IVB + Instrument Unit + adapter (after TLI; the fade shows the LM riding inside), the LM (lunar orbit), the SM (before re-entry). The camera tightens on what's left, then closes in on the command module, which turns slowly (`spinSpeed`). The beat list is content — parts, timing, cameras — and the order is the list's order. The presenter first listed the S-II before the tower; the real order is tower first, and that's what's built.
      **Keys:** Esc rewinds it (everything fades back, fast) while the camera flies home. ← or → drops it straight back onto card 04; the hotspot stays selected underneath while it plays, and the card and dimming just step aside. Number keys, X, C, S and Start Presentation drop it at once.
      **How it works:** a pure function of one timeline `u`, like the TD&E, built from the same parts (`useSpacecraftParts` / `DockedLM` / `rigFor` / `uAt` / `timeAt`, now exported from `Spacecraft.tsx`). `csm.ts` merges the SM and CM into separate groups so the SM can go alone. The LM sits lower than in TD&E: the stowed model is too tall for the adapter and only fits because TD&E never shows it under the bell, so here its tunnel sits just under the SPS bell and its folded legs sink into the IU, hidden until the IU fades. The floor's contact shadow fades with the S-IC (`Ground_Shadow`, live while it's up). Fading swaps each mesh to a transparent clone (`fade.ts`), and the clones persist between runs so their shaders stay compiled.
      **Pre-talk:** after loading, open 04, click What came home?, then Esc. The first run compiles its see-through shaders (and the CSM/LM's, if S hasn't run yet); it stalled for seconds on the software renderer.
