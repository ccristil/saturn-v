...
src/
  ...
  scene/
    ...
    livery.ts                  # repaints a guest into its real colours (the N1 ships all-white; olive below the shroud line, white above). Also gives the LM's gold foil a metallic sheen
    Spacecraft.tsx             # the docked CSM + LM floating beside the Saturn V's nose, to scale, doing a slow "barbecue roll". LM from the glb; CSM built from primitives (bare CM cone, SM, RCS quads, engine bell), docked at the LM's hatch (`LM_HATCH`, glb scene space)
...
- [x] **Spacecraft button (docked CSM + LM)** — fourth HUD button (and `S`). The Apollo spacecraft as it flew after the Saturn V: CSM docked nose-to-nose with the LM, floating beside the rocket's nose at true scale (units-per-metre from the live Saturn V via `measureSaturnV`), rolling slowly about its long axis. Everything tunable is `SPACECRAFT_VIEW` in `hotspots.ts`. **The LM's legs are deployed** — the Tinkercad export fuses them into its colour meshes, so they can't be folded — which matches lunar orbit just before undocking, not the coast out (legs were stowed then). The model's VHF antenna stands proud of the LM roof, so the docking tunnel (`NECK`) is a little longer than life to keep it out of the CM cone. Mutually exclusive with explode/compare/hotspots/deck; Esc, X, C, ←/→, 1–5 all close it. No contact-shadow re-bake: it floats ~100 units up, outside the shadow pass. `public/models/lunar-module.glb` is `gltf-transform optimize --compress false --palette false --simplify-ratio 0.15 --simplify-error 0.002` of the 13 MB / 450k-tri download (`apollo_11_lunar_module.glb`) → ~70k tris, 1.8 MB. `--palette false` matters: the default palette step merges all ten colours into one texture and the gold foil can no longer be found by material name.
