...
- [x] **Spacecraft button (docked CSM + LM)** — fourth HUD button (and `S`). The Apollo spacecraft as it flew after the Saturn V: CSM docked nose-to-nose with the LM, floating beside the rocket's nose at true scale (units-per-metre from the live Saturn V via `measureSaturnV`), rolling slowly about its long axis. Everything tunable is `SPACECRAFT_VIEW` in `hotspots.ts`. **The LM's legs are deployed** — the Tinkercad export fuses them into its colour meshes, so they can't be folded — which matches the last two hours docked in lunar orbit (Apollo 11's gear went down at 098:14:35 GET; Eagle undocked ~100:12), not the coast out (legs were stowed then). **The roll is presentation licence:** the barbecue roll was only ever flown with the gear stowed. Mutually exclusive with explode/compare/hotspots/deck; Esc, X, C, ←/→ and the number keys all close it.
